...
- Restart Python so 1.0.0 recreates with `tag_line`, `logo_filename`, `kiosk_theme`, and `business_fonts` in CREATE, and so live job events are sent. `scheduler.sqlite3` was deleted. Then re-run `verify a phone` and `pay a deposit`. Agents do not start or restart the service.
