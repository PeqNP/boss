...
public class TestManagementAPI {
    ...
    init() {
        self._home = bosslib.home(session:user:)
        self._search = bosslib.search(session:user:term:)

        self._project = bosslib.project(session:user:id:)
        self._saveProject = bosslib.saveProject(session:user:id:name:)
        self._deleteProject = bosslib.deleteProject(session:user:id:)
        self._projects = bosslib.projects(session:user:)
        self._testSuites = bosslib.testSuites(session:user:projectID:)
        self._projectTree = bosslib.projectTree(session:user:projectID:)

        self._testRun = bosslib.testRun(session:user:)
        self._findTestModels = bosslib.findTestModels(session:user:searchTerm:reverseLookup:)
        self._startTestRun = bosslib.startTestRun(session:user:name:includeAutomated:modelIDs:)
        self._activeTestRun = bosslib.activeTestRun(session:user:testRunID:)
        self._statusTestCase = bosslib.statusTestCase(session:user:testCaseResultID:status:notes:)
        self._finishTestRun = bosslib.finishTestRun(session:user:testRunID:determination:notes:)
        self._testRunResults = bosslib.testRunResults(session:user:id:)
        self._testRuns = bosslib.testRuns(session:user:)
        self._finishedTestRuns = bosslib.finishedTestRuns(session:user:)
        
        self._testSuite = bosslib.testSuite(session:user:id:)
        self._saveTestSuite = bosslib.saveTestSuite(session:user:id:projectID:name:)
        self._saveTestSuiteText = bosslib.saveTestSuite(session:user:id:text:testCases:)
        self._deleteTestSuite = bosslib.deleteTestSuite(session:user:id:)
        self._testCases = bosslib.testCases(session:user:testSuiteID:)
                
        self._testCase = bosslib.testCase(session:user:id:)
        self._saveTestCase = bosslib.saveTestCase(session:user:id:testSuiteID:name:notes:isAutomated:)
        self._deleteTestCase = bosslib.deleteTestCase(session:user:id:)
        
        self._createResource = bosslib.createResource(session:user:testSuiteID:)
        self._updateResource = bosslib.updateResource(session:user:id:name:mimeType:path:)
        self._deleteResource = bosslib.deleteResource(session:user:id:)
    }
