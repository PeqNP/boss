...
final class friendTests: XCTestCase {
    ...
    func testFriends() async throws {
        ...
        let user = try await api.account.saveUser(user: superUser(), id: nil, email: "eric@example", password: "Password1!", fullName: "Eric", verified: true, enabled: true)
        let mario = try await api.account.saveUser(user: superUser(), id: nil, email: "mario@example", password: "Password1!", fullName: "Mario", verified: true, enabled: true)
        let luigi = try await api.account.saveUser(user: superUser(), id: nil, email: "luigi@example", password: "Password1!", fullName: "Luigi", verified: true, enabled: true)
        let lonelyUser = try await api.account.saveUser(user: superUser(), id: nil, email: "lonely@example", password: "Password1!", fullName: "Lonely", verified: true, enabled: true)
        ...
        XCTAssertEqual(requests[safe: 1]?.email, "fake@example.com")
        XCTAssertEqual(requests[safe: 1]?.email, "mario@example.com")
        XCTAssertEqual(requests[safe: 2]?.email, "luigi@example.com")
        
        var friends = try await api.friend.friends(user: user)
        XCTAssertEqual(requests.count, 0, "it: should return no friends") // Sanity
        ...
        await XCTAssertError(
            try await api.friend.acceptFriendRequest(user: lonelyUser, id: user.id),
            api.error.FriendRequestNotFound()
        )
        
        // when: user accepts invalid friend request
        // NOTE: `id` belongs to the friend "who has no name"
        await XCTAssertError(
            try await api.friend.acceptFriendRequest(user: mario, id: user.id),
            api.error.FriendRequestNotFound()
        )
        
        // when: user accepts friend request
        try await api.friend.acceptFriendRequest(user: mario, id: 1)
        ...
        // when: user has no more friend requests
