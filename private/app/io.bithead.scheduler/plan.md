...
- **What is left:** leftovers in the app's `memory.md`. Every stage is finished and every flow in `ui-plan.md` has a spec.
...
| `/a/scheduler/config` | `"payment"` | `BusinessConfig` on the Payment tab. Stripe Connect returns here with `?code=` |
...
- [x] Kiosk scheduling flow (slots, session, OTP, confirm)
- [x] Appointment lookup by job code + verification code
- [x] Permanent lock after six failures, and every writer that has to honour it
- [x] Appointment modify/cancel
- [x] Admin schedule (month/week/day, assign week)
- [x] Job CRUD + payment
- [x] Job type CRUD
- [x] Employee CRUD + schedule templates + time-off
- [x] Time Slots mode + operating hours
- [x] Business config
