...
# The version of the last `create_version_*` that ran. Bump only when a
# different plan adds the next function to the chain in `start_database`.
...
BUSINESS_CONFIG_COLUMNS = """
    ...
    tag_line, logo_filename, kiosk_theme
"""
...
# same table and none of them belong to the owner. `logo_filename` is written
# by the logo routes, not the settings PUT.
BUSINESS_CONFIG_WRITABLE = frozenset(
    c.strip() for c in BUSINESS_CONFIG_COLUMNS.replace("\n", " ").split(",")
) - {"id", "is_active", "logo_filename"}
...
def set_business_logo(business_id: int, filename: Optional[str]) -> int:
    return update(
        "UPDATE businesses SET logo_filename = ?, update_date = datetime('now')"
        " WHERE id = ?",
