...
| 3 | Business settings | `scheduler-business-settings.spec.js` | Each tab saves and reads back — hours, slot mode, notice windows, confirmation channels; Enter in a field saves; an invalid value is refused with a message the screen shows; Payment says when Stripe keys are not configured; the chosen business type is inverted | **Done** — 9 specs |
| 4 | Job types | `scheduler-job-types.spec.js` | Create, name, size, Stripe product on a size, attribute, contact field, reorder, delete; a draft left unsaved does not reach the kiosk | **Done** — 11 specs |
...
| 6 | Kiosk booking | `scheduler-kiosk.spec.js` | The whole path a customer walks: service, size, employee, slot, contact, OTP, confirm — and the appointment exists afterwards with what they chose. Every kiosk page shows the Scheduler version and a `/a/scheduler` signup link | **Done** — 6 specs. OTP and deposit use mock vendors |
...
| 8 | Operator calendar | `scheduler-calendar.spec.js` | Month, week and day draw what was booked; a day opens; a job opens from it; assigning a week puts somebody on each; Queue stacks the day's remaining jobs and omits a completed one; Queue opens `QueueJob` (Complete, no Save, amount is the size cost, QR off without Stripe) and Complete marks the job done. Live redraw on `io.bithead.scheduler.job.changed` is the private suite plus the listener | **Done** — 7 specs |
...
| 5 | The Employee screen has no way to link a BOSS account. `plan.md` § 1.2 says it carries a "linked BOSS account (user search)", and `PUT /business/{id}/employee/{id}/account` is wired and tested — no screen calls it, so an operator cannot give an employee access to the app | **Yes** |
| 9 | An appointment cannot be written off. `lib.write_off_payment` is written and tested, no route calls it, and no screen offers it — so `payment_status` never becomes `written_off` and the Financial Report's Write-Offs figure can only ever read $0.00 | **Yes** |
