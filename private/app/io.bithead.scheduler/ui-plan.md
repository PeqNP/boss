...
| 8 | Operator calendar | `scheduler-calendar.spec.js` | Month, week and day draw what was booked; a day opens; a job opens from it; assigning a week puts somebody on each; Queue stacks the day's remaining jobs and omits a completed one. Live redraw on `io.bithead.scheduler.job.changed` is the private suite plus the listener | **Done** — 6 specs |
