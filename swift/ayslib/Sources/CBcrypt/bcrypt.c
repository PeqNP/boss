 ...
 */
int
boss_bcrypt_hashpass(const char *key, const char *salt, char *encrypted,
                      size_t encryptedlen)
{
    blf_ctx state;
    ...
    boss_Blowfish_initstate(&state);
    boss_Blowfish_expandstate(&state, csalt, salt_len,
                               (u_int8_t *) key, key_len);
    for (k = 0; k < rounds; k++) {
        boss_Blowfish_expand0state(&state, (u_int8_t *) key, key_len);
        boss_Blowfish_expand0state(&state, csalt, salt_len);
    }

    /* This can be precomputed later */
    j = 0;
    for (i = 0; i < BCRYPT_WORDS; i++)
        cdata[i] = boss_Blowfish_stream2word(ciphertext, 4 * BCRYPT_WORDS, &j);

    /* Now do the encryption */
    for (k = 0; k < 64; k++)
        boss_blf_enc(&state, cdata, BCRYPT_WORDS / 2);
    ...
    boss_encode_base64(encrypted + 7, csalt, BCRYPT_MAXSALT);
    boss_encode_base64(encrypted + 7 + 22, ciphertext, 4 * BCRYPT_WORDS - 1);
 ...
 */
int
boss_encode_base64(char *b64buffer, const u_int8_t *data, size_t len)
{
