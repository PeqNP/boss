...
- Language-specific norms:
  - Python 3.11 or newer: type hints, PEP 8, logging, context managers, dataclasses where appropriate.
...
- Include a mini threat model table with exactly 3 rows in the format: Vector -> Impact -> Mitigation
```
… -> … -> …
… -> … -> …
… -> … -> …
