...
|---|---|---|
| `commit-message.py` | `Stop` | Blocks a reply that changed a file and carries no commit message — a `<area>: <subject>` line and bullets. Blocking a `Stop` hands the reason back, so the message is written before the turn ends. Judged on what the reply did, not on the state of the tree: a tree dirty from an earlier turn was asked when that turn made the changes. |
